...
The chart coordinates are computed from a deterministic screening rubric rather than manually placed. For option \(i\), let \(T_i\) be the technology readiness score, \(A_i\) be the capacity factor from the cost model, \(M_i\) be the Mars operating score from \path{technology_screening_inputs.csv}, and \(C_i\) be the LCOE score:
...
The adapted deployability coordinate \(D_i\) and commercial operation coordinate \(P_i\) are then:
