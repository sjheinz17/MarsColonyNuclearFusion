...
\begin{table}[H]
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}p{2.0in}>{\raggedright\arraybackslash}p{4.0in}}
        \toprule
        File & Source and status \\
        \midrule
        \path{MDAD.csv} & Downloaded from the Mars Dust Activity Database public release \cite{mdad}. \\
        \path{nuclear_capacity_data_annual.csv} & Downloaded from U.S. Energy Information Administration Electric Power Annual Table 4.08.B \cite{eia}. \\
        \path{fusion_reactor_specs.csv} & Team assembled table based on SPARC \cite{sparc}, Princeton Satellite Systems material on direct fusion drive \cite{pss}, and Avalanche Energy Orbitron material \cite{avalanche}. \\
        \path{habitat_engineering_constants.csv} & Team built assumption table anchored to NASA Mars DRA 5.0 and MOXIE power related mission context \cite{dra,moxie}. \\
        \path{storage_specs.csv} & Team built benchmark table for storage assumptions used in the class model. \\
        \path{dust_penalty_curve.csv} & Team built seasonal solar derating curve used in the Mars year simulation. \\
        \bottomrule
    \end{tabular}
\end{table}
